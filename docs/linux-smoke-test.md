...
| Environment | Artifact | Shortcut | Window drag | Text paste | Manual fallback | Result |
|---|---|---|---|---|---|---|
| GNOME Wayland with Xwayland | AppImage and deb | Native GNOME + resident process | Required | `ydotool` | Required | Pending |
| KDE Plasma Wayland | AppImage | Global Shortcuts portal | Required | `ydotool` or `wtype` | Required | Pending |
| GNOME or Plasma X11 | AppImage | Electron/X11 | Required | `xdotool` | Required | Pending |
| ARM64 Linux graphical session | ARM64 AppImage or deb | Desktop-specific | Required | Available bridge | Required | Pending |
